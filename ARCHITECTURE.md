...
1. `K8S/sqlserver-secrets.yaml`
2. `K8S/sqlserver-platforms-deploy.yaml` (or equivalent SQL deployment)
3. `K8S/rabbitmq-deploy.yaml`
4. `K8S/platforms-deploy.yaml`
5. `K8S/commands-deploy.yaml`
6. `K8S/platforms-nodeport.yaml`
7. `K8S/ingress-srv.yaml`
...
SVG asset: [ARCHITECTURE.md](ARCHITECTURE.md)
...
flowchart TD
  subgraph K8s[Kubernetes Cluster]
    ...
    subgraph Infra[Stateful Infrastructure]
      MSSQL[(MSSQL StatefulSet)]
      Rabbit[(RabbitMQ StatefulSet)]
    end
  end

  %% Traffic Flow
  Ingress -->|/platforms| PlatformDeploy
  
  %% Platform interactions
  PlatformDeploy --> MSSQL
  PlatformDeploy -->|Sync Call| CommandsDeploy
  PlatformDeploy -->|Publish| Rabbit
  
  %% Commands interactions
  Rabbit -->|Subscribe| CommandsDeploy
  CommandsDeploy --> InMemory
```

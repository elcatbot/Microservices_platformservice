...
## Run with Kubernetes (minikube / cluster)

Apply manifests from the `K8S/` folder in the order described in `ARCHITECTURE.md`:

```bash
# apply secrets and DB first
kubectl apply -f K8S/mssql-secret.yaml
kubectl apply -f K8S/mssql-plat-depl.yaml
# then messaging and services
...
If you'd like, I can also create a `.env.example` with the common environment variables used by both services and a short `docker-compose.override.yml` for local developer overrides. Would you like that created now?
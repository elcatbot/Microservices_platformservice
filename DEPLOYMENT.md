...
2. Update Kubernetes manifests values to reference the pushed image tags.

## Kubernetes (K8S)

Files: the repo contains manifests under `K8S/`. Review each manifest before applying.

1. Prepare secrets (example): create or edit `K8S/sqlserver-secret.yaml` with production-safe credentials (do not commit real secrets).
...
kubectl apply -f K8S/sqlserver-platforms-deploy.yaml
kubectl apply -f K8S/rabbitmq-deploy.yaml
kubectl apply -f K8S/platforms-deploy.yaml
kubectl apply -f K8S/commands-deploy.yaml
